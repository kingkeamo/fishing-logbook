...
| `FishingLogBook.Application` | Application services and contracts (`I*Repository`, `IDatabaseMigrator`). |
| `FishingLogBook.Infrastructure` | Dapper repositories, Npgsql connection factory, DbUp migrator. |
...
**Dapper + Npgsql**, migrations via **DbUp** (SQL scripts under `database/migrations/`,
embedded into `FishingLogBook.Infrastructure`). Table names must **not** contain
underscores.
...
- Register services in `DependencyInjection.AddApplication()`.
...
- Register via `DependencyInjection.AddInfrastructure(IConfiguration)`.
