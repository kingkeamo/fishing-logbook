...
public sealed class PhotographMetadataService : IPhotographMetadataService
{
    ...
    public PhotographMetadataService(ITimeService time)
    {
        _time = time;
    }

    public async Task<PhotographMetadataModel> ReadAsync(
        byte[] bytes,
        string contentType,
        ...
        bool bigEndian)
    {
        ...
        if (Math.Abs(latitude.Value) > MaxLatitudeDegrees
            || Math.Abs(longitude.Value) > MaxLongitudeDegrees
