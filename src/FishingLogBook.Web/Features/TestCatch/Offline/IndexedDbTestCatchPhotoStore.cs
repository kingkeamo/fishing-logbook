...
public sealed class IndexedDbTestCatchPhotoStore : ITestCatchPhotoStore
{
    ...
    public async Task<TestCatchPhotoBytesModel?> GetAsync(Guid testCatchId, CancellationToken cancellationToken)
    {
        return await OfflineOperation.ExecuteAsync(
            ...
            async token =>
            {
                ...
                return new TestCatchPhotoBytesModel(Convert.FromBase64String(stored.BytesBase64), stored.ContentType);
            },
            cancellationToken,
            _logging);
    }
