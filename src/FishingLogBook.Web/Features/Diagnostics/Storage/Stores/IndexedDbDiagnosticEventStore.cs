...
namespace FishingLogBook.Web.Features.Diagnostics.Storage.Stores;
...
public sealed class IndexedDbDiagnosticEventStore : IDiagnosticEventStore
{
    ...
    public async Task<IReadOnlyList<DiagnosticEventModel>> GetPendingAsync(int maxCount, CancellationToken cancellationToken)
    {
        var items = await WithTimeoutAsync(
            (module, token) => module.InvokeAsync<string[]>("getPendingDiagnosticEvents", token, maxCount).AsTask(),
            cancellationToken);
        return (items ?? [])
            .Select(json => JsonSerializer.Deserialize<DiagnosticEventModel>(json, JsonOptions))
            .Where(item => item is not null)
            .Cast<DiagnosticEventModel>()
            .ToArray();
    }
    ...
    public async Task<DiagnosticDatabaseInspectionModel> InspectExistingAsync(CancellationToken cancellationToken)
    {
        var inspection = await WithTimeoutAsync(
            (module, token) => module.InvokeAsync<DiagnosticDatabaseInspectionModel?>(
                "inspectExistingDiagnosticDatabase",
                token).AsTask(),
            cancellationToken);
        return inspection ?? new DiagnosticDatabaseInspectionModel();
    }
