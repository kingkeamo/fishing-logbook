...
namespace FishingLogBook.Web.Features.Trips.Pages.OfflineTripEdit;

public partial class OfflineTripEdit : ComponentBase
{
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            ...
            _catches = await LoadCatchesAsync(owner.UserId);
            await ReadPreferencesAsync();
        }
        catch (Exception exception)
        {
            ...
            await Logging.LogErrorAsync(
                "loading a trip for editing offline",
                exception,
    ...
    private async Task<IReadOnlyList<CatchModel>> LoadCatchesAsync(Guid ownerUserId)
    {
        ...
        {
            await Logging.LogErrorAsync(
                "reading the catches of a trip for editing offline",
                exception,
                CancellationToken.None);
            return [];
        }
    ...
    private async Task ReadPreferencesAsync()
    {
        try
        {
            var preferences = await AnglerPreferences.GetAsync(CancellationToken.None);
        ...
        {
            await Logging.LogErrorAsync(
                "reading angler preferences for a trip offline",
                exception,
                CancellationToken.None);
        }
    }
    ...
    {
        Navigation.NavigateTo($"/offline/trips/{TripId:D}");
    }
}
